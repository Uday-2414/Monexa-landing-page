...
const CardNav: React.FC<CardNavProps> = ({
  ...
  return (
    ...
    >
      <nav
        ref={navRef}
        className={`card-nav ${isExpanded ? 'open' : ''} block h-15 p-0 rounded-xl shadow-md relative overflow-hidden will-change-[height] transition-colors duration-300`}
      ...
      >
        <div className="card-nav-top absolute inset-x-0 top-0 h-15 flex items-center justify-between p-2 pl-[1.1rem] z-2">
          ...
          <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-0">
            {/* Display transparent CK logo SVG */}
            <img src={logo} alt={logoAlt} className="logo h-8.5 transition-transform duration-300 hover:scale-105" />
          </div>

          <div className="flex items-center gap-3 order-3 md:order-0">
            {/* Theme Toggle Button next to Get Started button */}
            <button
              onClick={onToggleTheme}
              type="button"
              className="p-2.5 rounded-lg border border-slate-200/80 dark:border-purple-500/20 bg-slate-100/55 dark:bg-purple-950/20 text-slate-800 dark:text-purple-300 hover:bg-slate-200/80 dark:hover:bg-purple-950/40 transition duration-300 cursor-pointer flex items-center justify-center shadow-sm"
            ...
            <button
              type="button"
              className="card-nav-cta-button hidden md:inline-flex border-0 rounded-[0.55rem] px-4 items-center h-full font-medium cursor-pointer transition-colors duration-300 hover:brightness-105"
